...
exports.drawLines = (image, color, lines) => {
  ...
  for (let i = 0; i < lines.length; ++i) {
    ...
    for (let j = line.x1; j < line.x2; ++j) {
      ...
      data[o + 0] = ((dr * a + sr * ma) / m >> 8) | 0
      data[o + 1] = ((dg * a + sg * ma) / m >> 8) | 0
      data[o + 2] = ((db * a + sb * ma) / m >> 8) | 0
      data[o + 3] = ((da * a + sa * ma) / m >> 8) | 0
    }
  }
}
/*
exports.drawLines = (image, color, lines) => {
  ...
  for (let i = 0; i < lines.length; ++i) {
    const line = lines[i]
    const ma = Math.max(0, Math.min(1, line.alpha * color.a / (255 * 255)))
    const a = 1.0 - ma
    ...
    for (let j = line.x1; j < line.x2; ++j) {
      ...
      data[o + 0] = sr // (dr * a + sr * ma) | 0
      data[o + 1] = sg // (dg * a + sg * ma) | 0
      data[o + 2] = sb // (db * a + sb * ma) | 0
      data[o + 3] = sa // (da * a + sa * ma) | 0
    }
  }
}
*/
